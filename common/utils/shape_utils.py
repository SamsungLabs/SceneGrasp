...
from common.utils.misc_utils import get_data_dump_stamp
...
def init_logs_dir_from_path(logs_dir: Path, logger_name: str, input_args: Dict = {}):
    logger = setup_logger(logger_name, logs_dir / "log.txt")
    logger.info(f"Init logs directory at {logs_dir}")
    logger.info("Input args: ")
    for key, value in input_args.items():
        logger.info(f"\t- {key}: {str(value)}")
    return logs_dir, logger


def init_logs_dir(
    logs_root: Path, logger_name: str, logs_dir_suffix: Union[str, None] = None, input_args: Dict = {}
) -> Tuple[Path, logging.Logger]:
    # Create a logs directory with date stamp
    logs_dir_name = get_data_dump_stamp()
    if logs_dir_suffix is not None:
        logs_dir_name = f"{logs_dir_name}_{logs_dir_suffix}"
    logs_dir = logs_root / logs_dir_name
    logs_dir.mkdir(parents=True)
    return init_logs_dir_from_path(logs_dir, logger_name, input_args)
...
def sample_points_from_mesh_vert_face(vertices, faces, n_pts, with_normal=False, fps=False, ratio=2):
    if fps:
...
def sample_points_from_mesh(path, n_pts, with_normal=False, fps=False, ratio=2):
    ...
    return sample_points_from_mesh_vert_face(vertices, faces, n_pts, with_normal, fps, ratio)
